...
//上传头像函数
function uploadLogo(url, opt){
	opt = $.extend({
		picW: 200,
		picH: 150,
		block: 120,
		pic: $('#uploadpic'),
		form: $('#registrationPicForm')
	}, opt || {});
	if( opt.pic.length == 0 ){ return; }

	var picArea = {x: 0, y: 0, z: 1},
		w = parseInt(opt.pic.attr('w'), 10),
		h = parseInt(opt.pic.attr('h'), 10),
		c = {left: 0, top: 0};

	if( w <= h ){
		var scale = opt.block / w;
	} else {
		var scale = opt.block / h;
		if( w * scale > opt.picW ){
			scale = opt.picW / w;
		}
	}
	...
	opt.picW = Math.ceil(scale * w);
	opt.picH = Math.ceil(scale * h);
	opt.pic.css({width: opt.picW + 'px', height: opt.picH + 'px'});

	var block = Math.ceil(Math.min(opt.block, opt.picW, opt.picH));
	c.left = Math.ceil(Math.max(c.left, (opt.picW - block) / 2));
	c.top = Math.ceil(Math.max(c.top, (opt.picH - block) / 2));
	picArea.z = Math.max(opt.block / block, opt.block / opt.picW, opt.block / opt.picH, scale);
	picArea.x = block < opt.block ? c.left * picArea.z : c.left;
	picArea.y = block < opt.block ? c.top * picArea.z : c.top;
	
	var preview = function (img, selection) {
		var scaleX = opt.block / (selection.width || 1);
		var scaleY = opt.block / (selection.height || 1);
		$('#logoPreview > img').css({ width: Math.round(scaleX * opt.picW) + 'px', height: Math.round(scaleY * opt.picH) + 'px', marginLeft: '-' + Math.round(scaleX * selection.x1) + 'px', marginTop: '-' + Math.round(scaleY * selection.y1) + 'px' });
	};
	...
	opt.pic.imgAreaSelect({
		...
		onSelectEnd: function(img, selection){
			if( selection.width ){
				var s = opt.block / selection.width;
				picArea.z = Math.round((s * scale) * 100) / 100;
	...
	setTimeout(function(){
		preview(opt.pic, {width: block, height: block, x1: c.left, y1: c.top, x2: c.left + block, y2: c.top + block});
	}, 200);
	
	$('#logoPreview')
		.css({position: 'relative', overflow: 'hidden', width: opt.block + 'px', height: opt.block + 'px'})
		.append('<img src="' + opt.pic.attr('src') + '" style="position: relative;" />');

	$('#saveLogo').click(function(){
		url += '?x=' + Math.round(picArea.x) + '&y=' + Math.round(picArea.y) + '&z=' + picArea.z;
		setTimeout(function(){
			opt.form.attr('action', url).submit();
		}, 200);
	});
}
