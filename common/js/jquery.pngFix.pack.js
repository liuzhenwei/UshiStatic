...
$.fn.pngFix = function(settings) {
	...
	if ($.browser.msie && (ie55 || ie6)) {
		...
		if (settings.fixSelf == true) {
			if (_this.is('img')) {
				_this.each(fixImage);
			} else if (_this.is('input')) {
				_this.each(fixInput);
			} else {
				_this.each(fixBackground);
			}
		}

		_this.find("img[src$=.png]").each(fixImage);
		_this.find("*").each(fixBackground);
		_this.find("input[src$=.png]").each(fixInput);
